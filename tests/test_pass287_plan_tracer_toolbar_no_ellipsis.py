...
def test_plan_tracer_toolbar_uses_full_visible_labels_and_semantic_names() -> None:
    toolbox = _toolbox()
    ...
    assert [button.label for button in toolbox.buttons] == labels
    assert [button.display_label or button.label for button in toolbox.buttons] == labels
    assert toolbox.width_px == 1240
    assert _toolbar_content_width(toolbox) > 1100
    assert max(_toolbar_button_slot_widths(toolbox)) == 92


def test_plan_tracer_vector_renderer_does_not_ellipsis_builder_sized_labels() -> None:
    ...
    assert 'button.setProperty("toolCoreVectorNoElide", True)' in layout
    assert 'no_elide = bool(self.property("toolCoreVectorNoElide"))' in widgets
    assert 'builder has sized the slot from that caption' in widgets
